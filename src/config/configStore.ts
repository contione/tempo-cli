import fs from 'fs'
import os from 'os'
import { promisify } from 'util'
import path from 'path'
import { Tracker } from './trackerStore'

const readFileAsync = promisify(fs.readFile)
const writeFileAsync = promisify(fs.writeFile)
const renameAsync = promisify(fs.rename)
const chmodAsync = promisify(fs.chmod)

export type Config = {
    tempoToken?: string
    accountId?: string
    atlassianUserEmail?: string
    atlassianToken?: string
    hostname?: string
    aliases?: Map<string, string>
    trackers?: Map<string, Tracker>
}

export function configFilePath(): string {
    return path.join(os.homedir(), '.tempo-cli.json')
}

export default {

    async save(config: Config) {
        const configPath = configFilePath()
        const tempPath = `${configPath}.${process.pid}.tmp`
        const configJson = JSON.stringify(config, replacer)
        await writeFileAsync(tempPath, configJson, { mode: 0o600 })
        await chmodAsync(tempPath, 0o600)
        await renameAsync(tempPath, configPath)
    },

    async read(): Promise<Config> {
        try {
            const configJson = await readFileAsync(configFilePath(), { encoding: 'utf8' })
            return JSON.parse(configJson, reviver)
        } catch (error) {
            if ((error as { code?: string }).code === 'ENOENT') {
                return emptyConfig()
            }
            throw Error('The tempo-cli configuration file is unreadable or invalid. Run setup after fixing it.')
        }
    }
}

function emptyConfig(): Config {
    return {
        tempoToken: undefined,
        accountId: undefined,
        atlassianUserEmail: undefined,
        atlassianToken: undefined,
        hostname: undefined,
        aliases: undefined,
        trackers: undefined
    }
}

function replacer(this: any, key: any, value: any) {
    const originalObject = this[key]
    if (originalObject instanceof Map) {
        return {
            dataType: 'Map',
            value: Array.from(originalObject.entries())
        }
    } else {
        return value
    }
}

function reviver(key: any, value: any) {
    if (typeof value === 'object' && value !== null) {
        if (value.dataType === 'Map') {
            return new Map(value.value)
        }
    }
    return value
}
